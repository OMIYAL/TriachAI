# Release 24.0 Bug and Test Summary

**Release Milestone:** Release 24.0  
**Version**:  v1.26.3.25
**Date:** 2026-03-27
...
| #79   | TC-044       | Medium  | Halfway Query Interrupt (Queries Check) (B2C)|Close  | @dev1 | The interrupted query does not deduct a credit; the quota remains unchanged. |
...
| #104   | TC-057       | Medium  | Tenant Name Below QueryCounter  | Close | @dev1 |Tenant Name Below QueryCounter|
...
| #120   | TC-071       | Medium  | Payment History Filter Not Working| Close | @dev1 |The Payment History filter is not functioning properly. When users apply filter criteria, the results do not update accordingly.|
| #121  | TC-072       | Medium  | Query Session Discrepancy | Open | @dev1 | When accessing an existing Archai Assistant session directly via its URL (session ID), the session does not show recommended follow-up questions. Additionally, the jurisdiction name fails to load when the URL is opened in an an incognito tab or another browser, though it successfully loads if opened in a new tab within the same active browser session.|
| #122   | TC-073    | Medium   | Monthly Reset Not Working After Month Completion | open | @dev1 | The system is expected to reset the query automatically after the completion of each month. However, the reset is not occurring even after the new month begins.|
| #123   | TC-074    | Medium   | Preference Identification Missing in Session History and Bookmark Queries | open | @dev1 | Each query in Session History and Bookmark Queries should clearly indicate the preference it belongs to.|
| #124   | TC-075    | Medium   | Query counter issue| open | @dev1 | The Queries popup should just close automatically when you tap anywhere else on the screen|
